...
class BasicModule(nn.Module):
    def __init__(self, conv, n_feat, kernel_size, block_type='residual', bias=True,
        ...
        if block_type == 'basic':
            ...
            n_blocks = 5
            m_body = [
                common.ResBlockWithCSA(conv, n_feat, kernel_size)
                for _ in range(n_blocks)
            ]
        else:
...
class CSARST(nn.Module):

    def __init__(self, args, conv=common.default_conv):
        super(CSARST, self).__init__()
