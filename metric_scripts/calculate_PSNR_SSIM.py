...
def main():
    ...
    folder_GT = './dataset/RSCNN7/test/HR_x4'
    folder_Gen = './experiment/CSARST_RSCNN7X4/results'
    ...
    for i, img_path in enumerate(img_list):
        ...
        print('{:3d} - {:25}. \tPSNR: {:.6f} dB, \tSSIM: {:.6f}, \tSCC: {:.6f}, \tSAM: {:.6f}'.format(
            i + 1, base_name, PSNR, SSIM, SCC, SAM))
        PSNR_all.append(PSNR)
        SSIM_all.append(SSIM)
        SCC_all.append(SCC)
        SAM_all.append(SAM)

        print('Average: PSNR: {:.6f} dB, SSIM: {:.6f}, SCC: {:.6f}, SAM: {:.6f}'.format(
